...
public class DefaultPseudonymTerm extends AbstractTerm {

	private final ITerm baseTerm;

	protected DefaultPseudonymTerm(LocationInfo location, IScope scope, ITerm baseTerm) {
	...
	public IType inferType() {
		return getScope().findFunction(Prelude.DEFAULT_PSEUDONYM).getType();
	}

	public ITerm reduce(SymbolsState symState) {
		return new DefaultPseudonymTerm(getLocation(), getScope(), baseTerm.reduce(symState));
	}

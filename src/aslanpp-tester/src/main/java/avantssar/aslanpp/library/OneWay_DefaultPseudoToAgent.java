...
@Specification
public class OneWay_DefaultPseudoToAgent extends OneWay_AgentToAgent {
	...
	protected IntroduceStatement aslanppSend() {
		return alice.comm(alice.getActorSymbol().term().defaultPseudonym(), vAliceB.term(), cppToken.term(), null, channelType, false, false, false);
	}
	...
	protected org.avantssar.aslan.ITerm translateSend() {
		Function fncDefaultPseudonym = spec.findFunction(Prelude.DEFAULT_PSEUDONYM);
		return doSend(cm, vAlice_Actor.term(), fncDefaultPseudonym.term(vAlice_Actor.term(), vAlice_ID.term()), vAlice_B.term(), cToken.term());
	}

...
@Specification
public class ChannelGoalSecrecyAndAuthenticationJoined extends ChannelGoalSecrecyAndAuthentication {
	...
	protected void finishPPmodel() {
		...
		snd.attachChannelGoal(entAlice.chGoal(getPPprotName(), entAlice.getActorSymbol().term(), vAliceBob.term(), vAliceM.term(), ChannelEntry.secure));
		...
		rcv.attachChannelGoal(entBob.chGoal(getPPprotName(), vBobAlice.term(), entBob.getActorSymbol().term(), vBobM.term(), ChannelEntry.secure));
	}
	...
	protected void finishModel(IASLanSpec spec, ChannelModel cm) {
		...
		Function fAliceSet = spec.function(getSecrecyPrefix() + "_set", spec.setType(IASLanSpec.AGENT), IASLanSpec.NAT);
		Function fBobSet = spec.function(getSecrecyPrefix() + "_set", spec.setType(IASLanSpec.AGENT), IASLanSpec.NAT);
		Constant cSecProt = spec.constant(getSecrecyPrefix(), IASLanSpec.PROTOCOL_ID);
		Constant cAuthProt = spec.constant(getAuthPrefix(), IASLanSpec.PROTOCOL_ID);
	...
	protected void addAuthGoal(IASLanSpec spec) {
		Variable vMsg = spec.variable("E_aasp_Msg", IASLanSpec.MESSAGE);
		Variable vReq = spec.variable("Req", IASLanSpec.AGENT);
		Variable vWit = spec.variable("Wit", IASLanSpec.AGENT);
		Variable vIID = spec.variable("E_aasp_IID", IASLanSpec.NAT);
		Constant cProt = spec.findConstant(getAuthPrefix());

		AttackState as = spec.attackState(getAuthPrefix());
	...
	protected void addSecrecyGoal(IASLanSpec spec) {
		...
		Constant cProt = spec.findConstant(getSecrecyPrefix());
		AttackState as = spec.attackState("secr_" + getPPprotName());
	...
	protected String getPPprotName() {
		return "auth_sec_payload";
	}

	protected String getSecrecyPrefix() {
		return "secr_" + getPPprotName();
	}

	protected String getAuthPrefix() {
		return "auth_" + getPPprotName();
	}
